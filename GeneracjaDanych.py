...
fakty_labels = ["Produkt_ID", "Lokalizacja_ID", "Data_ID", "Platnosc_ID", "Wartosc", "Ilosc", "Rabat"]
with open(f'{file_dir}/Fakty.csv', 'w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(fakty_labels)
    writer.writerows(fakty)

produkty_row = ["Produkt_ID", "Nazwa_Produktu", "Podkategoria", "Kategoria", "CenaBazowa"]
produkty = []
for i, product in enumerate(Products):
    produkty.append(
        [i + 1, product.name, product.subcategory.name, kategorie2[product.subcategory.value].name, product.value])

with open(f'{file_dir}/Produkty.csv', 'w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(produkty_row)
    writer.writerows(produkty)

lokalizacje_row = ["Lokalizacja_ID", "Nazwa_Lokalizacji"]
...
with open(f'{file_dir}/Lokalizacje.csv', 'w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(lokalizacje_row)
    writer.writerows(lokalizacje_tabela)

platnosci_row = ["Platnosc_ID", "Nazwa_Metody"]
platnosci_tabela = [[p.value, p.name] for p in Platnosci]

with open(f'{file_dir}/Platnosci.csv', 'w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(platnosci_row)
    writer.writerows(platnosci_tabela)

daty_row = ["Data_ID", "Data", "Rok", "Miesiac", "Dzien", "DzienTygodnia"]
daty_tabela = []
...
with open(f'{file_dir}/daty.csv', 'w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(daty_row)
    writer.writerows(daty_tabela)

print("Dane wygenerowane i zapisane pomyslnie")